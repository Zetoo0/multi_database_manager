...
pub struct MySqlService {
    db_repo: Arc<MySqlRepository>,
}

impl MySqlService {
    pub fn new(connection_info: DatabaseConnection) -> MySqlService {
        let repo = Arc::new(MySqlRepository::new(connection_info));
        MySqlService { db_repo: repo }
    }
}
...
fn create_index<'a>(
    ...
    let db_repo = Arc::clone(&self.db_repo);
    Box::pin(async move {
        let index_create = CreateIndexInfo{
            table_name: index.table_name.unwrap(),
            index_name: index.name,
            columns: index.column_name.unwrap(),
            schema_name: index.schema_name.unwrap(),
        };
        let result_ = db_repo.create_index(&index_create, &db_name).await;
        result_
        //Ok(())
    })
}
...
fn create_sequence<'a>(
    ...
    let db_repo = Arc::clone(&self.db_repo);
    Box::pin(async move {
        //let result_ = db_repo.create_sequence(&seq.name, &db_name,).await;
        //result_
        todo!()
    })
}
...
fn create_trigger<'a>(
    ...
    let db_repo = Arc::clone(&self.db_repo);
    Box::pin(async move {
        let result_ = db_repo.create_trigger(name, when, type_, table_name, function_name, &db_name,&db_name).await;
       //todo!()
        result_
    })
}
fn create_view<'a>(
   ...
   //         let new_column = db_repo.alter_table_column(&table_name,new_cols.get(0).unwrap().clone(),old_cols.get(0).unwrap().clone(),"huwa").await.await?;
    ...
    fn create_constraint<'a>(
        ...
        let db_repo = Arc::clone(&self.db_repo);
        Box::pin(async move {
            let result_ = db_repo.create_constraint(constraint, table_name, schema_name).await.await;
            result_
        })
    }
}
