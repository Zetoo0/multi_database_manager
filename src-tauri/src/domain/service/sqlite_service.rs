use crate::domain::create_info::create_table_info::{CreateSequenceInfo, CreateViewInfo};
...
use crate::domain::repository::sqlite_repository::SqLiteRepository;
...
pub struct SqLiteService {
    db_repo: Arc<SqLiteRepository>,
}

impl SqLiteService {
    pub fn new(connection_info: DatabaseConnection) -> SqLiteService {
        let repo = Arc::new(SqLiteRepository::new(&connection_info.server.as_str()));
        SqLiteService { db_repo: repo }
    }
}
...
impl DatabaseService for SqLiteService {
    ...
    fn create_table<'a>(
        ...
        let db_repo = Arc::clone(&self.db_repo);
        Box::pin(async move{
            let result_ = db_repo.create_table(db_name, &table_info).await;
            result_
        })
    }
...
fn create_trigger<'a>(
    ...
    let db_repo = Arc::clone(&self.db_repo);
    Box::pin(async move{
        let result_ = db_repo.create_trigger(name, when, type_, table_name, function_name, &db_name).await;
        result_
    })
}
fn create_view<'a>(
        ...
        let db_repo = Arc::clone(&self.db_repo);
        Box::pin(async move{
            let view_info: CreateViewInfo = CreateViewInfo{
    ...
    fn edit_table_column<'a>(
        ...
        let db_repo = Arc::clone(&self.db_repo);
        Box::pin(async move{
            let result_ = db_repo.alter_table_column(table_name.as_str(), new_col, old_col, &db_name).await.await;
            result_
        })
    }
