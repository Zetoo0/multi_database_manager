use crate::metadata::{function::Function, procedure::Procedure, table:: Table, trigger, view::View};
...
impl MySqlRepository{
    pub fn new(connection_info:DatabaseConnection) -> Self{
        ...
        let base_url = String::from(format!("{}://{}:{}@{}:{}/",connection_info.driver_type,
                                        connection_info.username,connection_info.password,
                                        connection_info.server,connection_info.port));
        
        return MySqlRepository{ rb_map, base_url, databases, connection_info};
    }
...
impl DatabaseRepository for MySqlRepository{
        ...
        if let Some(tables) = result.as_array(){
           // let mut db_struct = self.databases.lock().unwrap();
            if let Some(mut node) = self.databases.get_mut(db_name){
               // let mut node_ref = node.value_mut();
                let table_map = node.value_mut().tables.get_or_insert_with(HashMap::new);
                for table in tables{
                    if let Value::Map(tablemap) = table{
                        println!("table name: {:?}",table_namae);
                        let table_name = tablemap.0.get(&Value::String(table_namae.clone())).and_then(|v| v.as_str()).unwrap_or_default();
    ...
    async fn get_columns(&self,db_name:&str,table_name:&str)-> Result<Value,rbdc::Error> {
        ...
        let result = rb.query(_sql.as_str(), vec![]).await.unwrap();
                ...
                if let Some(table_map) = node.value_mut().tables.clone().unwrap().get_mut(db_name){
                    ...
                    for col in cols{
                        if let Value::Map(colmap) = col{
                            let col_name = colmap.0.get(&Value::String("Field".to_string())).and_then(|v| v.as_str()).unwrap_or_default();
                            let data_type = colmap.0.get(&Value::String("Type".to_string())).and_then(|v| v.as_str()).unwrap_or_default();
                            let is_nullable = colmap.0.get(&Value::String("Null".to_string())).and_then(|v| v.as_bool()).unwrap_or_default();
                            let column_default = colmap.0.get(&Value::String("Default".to_string())).and_then(|v| v.as_str()).unwrap_or_default();
            ...
            if let Some(mut node) = self.databases.get_mut(db_name){
                ...
                for view in views{
                    if let Value::Map(viewmap) = view{
                        let view_name = viewmap.0.get(&Value::String("Tables_in_".to_string()+db_name)).and_then(|v| v.as_str()).unwrap_or_default();
                        let view_definition = viewmap.0.get(&Value::String("Table_type".to_string())).and_then(|v| v.as_str()).unwrap_or_default();
                        let view_node = View{
    ...
    async fn get_stored_procedures(&self,db_name:&str)-> Result<Value,rbdc::Error> {
        ...
        let _sql = format!("SHOW PROCEDURE STATUS WHERE Db = {}",db_name);
        let result = rb.query(_sql.as_str(), vec![]).await.unwrap();
            ...
            if let Some(mut node) = self.databases.get_mut(db_name){
                ...
                for stored_procedure in stored_procedures{
                    if let Value::Map(stored_proceduremap) = stored_procedure{
                        let stored_procedure_name = stored_proceduremap.0.get(&Value::String("Name".to_string())).and_then(|v| v.as_str()).unwrap_or_default();
    ...
    async fn get_functions(&self,db_name:&str)-> Result<Value,rbdc::Error> {
        ...
        let _sql = format!("SHOW FUNCTION STATUS WHERE Db = '{db_name}';");
        let result = rb.query(_sql.as_str(), vec![]).await.unwrap();
            ...
            if let Some(mut node) = self.databases.get_mut(db_name){
                ...
                for function in functions{
                    if let Value::Map(functionmap) = function{
                        let function_name = functionmap.0.get(&Value::String("Name".to_string())).and_then(|v| v.as_str()).unwrap_or_default();
                        let function_schema = functionmap.0.get(&Value::String("Definer".to_string())).and_then(|v| v.as_str()).unwrap_or_default();
            ...
            if let Some(mut node) = self.databases.get_mut(db_name){
                ...
                for trigger in triggers{
                    if let Value::Map(triggermap) = trigger{
                        let trigger_name = triggermap.0.get(&Value::String("Trigger".to_string())).and_then(|v| v.as_str()).unwrap_or_default();
    ...
    async fn get_indexes(&self,db_name:&str,table_name:&str)-> Result<Value,rbdc::Error> {
        ...
        let _sql = "SELECT 
                    ...
                    WHERE TABLE_SCHEMA = '?';";
        let result = rb.query(_sql, vec![db_name.into()]).await.unwrap();
                ...
                if let Some(table_map) = node.value_mut().tables.clone().unwrap().get_mut(db_name){
                    ...
                    for index in indexes{
                        if let Value::Map(indexmap) = index{
                            ...
                            let index_definition = indexmap.0.get(&Value::String("index_type".to_string())).and_then(|v| v.as_str()).unwrap_or_default();
                ...
                if let Some(table_map) = node.value_mut().tables.clone().unwrap().get_mut(table_name){
                    ...
                    for constraint in constraints{
                        if let Value::Map(colmap) = constraint{
                            let constraint_name = colmap.0.get(&Value::String("Table".to_string())).and_then(|v| v.as_str()).unwrap_or_default();
                            let constraint_type = colmap.0.get(&Value::String("Create Table".to_string())).and_then(|v| v.as_str()).unwrap_or_default();
    ...
    async fn get_partitions(&self,db_name:&str,table_name:&str)-> Result<Value,rbdc::Error> {
        ...
        let result = rb.query(_sql.as_str(), vec![Value::String(table_name.to_string())]).await.unwrap();
        Ok(result)
    }
    ...
    async fn get_user_privileges(&self,db_name:&str,user_name:&str)-> Result<Value,rbdc::Error> {
        ...
        let _sql = format!("SHOW GRANTS FOR '{}'@'{}';",user_name,self.connection_info.server);
        let result = rb.query(_sql.as_str(), vec![Value::String(user_name.to_string())]).await.unwrap();
        Ok(result)
    }
